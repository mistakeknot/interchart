...
def test_refuses_a_collapsed_scan_below_the_absolute_floor(project_root, tmp_path):
    ...
    result = _run(project_root / "scripts" / "generate.sh", root, out,
                  env={**os.environ, "INTERCHART_ALLOW_MISSING": "1"})
...
def test_refuses_when_an_input_directory_is_missing(tmp_path, project_root):
    """Guard 0: a missing input is fatal, because no count threshold can see it.

    This is the 219-vs-244 case. scan.js looked for os/clavain, Linux did not
    resolve the case, the hub vanished, and the artefact was 10% smaller — well
    inside every ratio guard. The only reliable signal was that an input the
    scanner asked for was not there.
    """
    import os

    root = tmp_path / "root"
    (root / "interverse").mkdir(parents=True)
    (root / "os").mkdir()  # exists, so guard 1 passes; Clavain absent inside it
    out = tmp_path / "out.html"

    result = _run(project_root / "scripts" / "generate.sh", root, out)

    assert result.returncode == 4, result.stderr
    assert "could not see all its inputs" in result.stderr
    assert "os/Clavain not found" in result.stderr
    assert not out.exists(), "nothing may be written when an input is missing"

    # The escape hatch lets a legitimately partial checkout through, and the
    # remaining guards still apply to it.
    allowed = _run(project_root / "scripts" / "generate.sh", root, out,
                   env={**os.environ, "INTERCHART_ALLOW_MISSING": "1"})
    assert allowed.returncode == 3, allowed.stderr
    assert "below the absolute floor" in allowed.stderr
